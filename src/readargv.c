...
void printHelp()
{
  ...
  printf("\t-L  <librarys_file>: Library file from alignment to applying the consistency scores\n");
  printf("\t-l  <factor_value>: Library factor value to apply to exon scores\n");
  printf("\t-q  <normalization_value>: Turn on/off the normalization of the library scores\n");
  printf("\t-c  <number_cycles>: Number of relaxation cycles to the library\n");
	       ...
	       char* ExonsFile, char* HSPFile, char* LibraryFile, char* GenePrefix)
{
  ...
  while ((c = getopt(argc,argv,"oO:bdaefitnsrxj:k:N:p:UDATzZXmMG3BvE:V:R:S:WCFP:huL:l:c:q:")) != -1)
    switch(c)
      {
      case 'B': BEG++; 
		break;
      ...
      case 'z': EOP++;
		...
		break;
      case 'L': LIB++;
		strcpy (LibraryFile,optarg);
		geneidOpts++;
		printOptions++;
		break;
      case 'l':
    	LIBF = strtof(optarg,&dummy4);
		geneidOpts++;
		printOptions++;
		break;
      case 'c':
    	CYCLES = strtol(optarg,&dummy5,0);
		geneidOpts++;
		printOptions++;
		break;
      case 'q':
    	NORM = strtol(optarg,&dummy6,0);
		geneidOpts++;
		printOptions++;
		break;
