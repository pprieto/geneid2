...
int
  ...
  /* Using library from tcoffee alignment */
...
int main (int argc, char *argv[])
{
  ...
  int lastSplit;
  char mess[MAXSTRING];
  struct timespec requestTotalStart;
  struct timespec requestTotalEnd;
  struct timespec requestStart;
  struct timespec requestEnd;
  struct timespec requestMemoryStart;
  struct timespec requestMemoryEnd;
  double accum;

  clock_gettime( CLOCK_MONOTONIC, &requestTotalStart );
  ...
  readargv(argc,argv,ParamFile,SequenceFile,ExonsFile,HSPFile,LibraryFile,GenePrefix);
  ...
  clock_gettime(CLOCK_MONOTONIC, &requestMemoryStart);
  external      = (packExternalInformation*) RequestMemoryExternalInformation();
  clock_gettime(CLOCK_MONOTONIC, &requestMemoryEnd);
  accum = ( requestMemoryEnd.tv_sec - requestMemoryStart.tv_sec )
    + ( requestMemoryEnd.tv_nsec - requestMemoryStart.tv_nsec )
    / BILLION;
  //printf( "Memory alloc consumes: %lf\n", accum );
  ...
  if (GENEID)
      ...
      if (SRP)
	{
	  ...
	  clock_gettime(CLOCK_MONOTONIC, &requestTotalEnd);
	  accum = ( requestTotalEnd.tv_sec - requestTotalStart.tv_sec )
	    + ( requestTotalEnd.tv_nsec - requestTotalStart.tv_nsec )
	    / BILLION;
	  //printf( "%lf\n", accum );
      if(LIB)
      {
    	  //printMess("Reading library information...");
    	  //clock_gettime( CLOCK_MONOTONIC, &requestStart );
    	  external->nTotPairs = ReadLibrary(LibraryFile, external);
    	  clock_gettime(CLOCK_MONOTONIC, &requestTotalEnd);
    	  ...
    	  accum = ( requestTotalEnd.tv_sec - requestTotalStart.tv_sec )
    	    + ( requestTotalEnd.tv_nsec - requestTotalStart.tv_nsec )
    	    / BILLION;
    	  //printf( "End of reading file: %lf\n", accum );

    	  sprintf(mess,"%ld Pairs acquired from file", external->nTotPairs);
		  //printMess(mess);

		  printMess("Testing the library...");
		  //testLibrary(external);
		  testLibrary2("output_lib.tc_lib", external);
		  //printMess("Done");
		  //exit(1);
      }
      if (EVD || SRP || LIB)
	{
	  ...
	  if(LIB)
	  {
		  ...
		  //RelaxLibrary(external, Locus);
		  clock_gettime(CLOCK_MONOTONIC, &requestStart);

		  PreProcessLibrary(external);
      ...
      while (reading != EOF)
	{
	  ...
	  if(LIB)
	  {
		  printMess("Select library information");
		  lib = SelectLibrary(external, Locus, LengthSequence);
		  if(lib == NULL)
			  sprintf(mess, "No information has been provided for %s\n",Locus);
		  else
			  sprintf(mess,"Using pairs in %s",Locus);
		  printMess(mess);
	  }
	  if (SRP)
	    {
