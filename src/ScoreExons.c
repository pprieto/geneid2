...
/* Library information to protein score: using library information (pairs)*/
float ScoreLibraryExon(exonGFF* exon, int Strand, packExternalInformation* external, long l1, long l2)
{
	  int index;
	  short trueFrame;
	  long iniExon, endExon;
...
/* 		p = gp->utr; */
      ...
      if (scoreMarkov >= p->OligoCutoff) 
	{
	  ...
	  if (SRP){
            ...
            sprintf(mess,"score: %.5f, exon: %d",scoreHSP,i);
            printMess(mess);
	  }
...
/* 	    printMess(mess); */
	  ...
	  if(LIB)
	  {
		  scoreLib = ScoreLibraryExon((Exons+i), Strand, external, l1, l2);
	  }
