...
/* Max number of sequences in the library */
#define MAXSEQLIB 20
/* Max number of length for library sequences*/
#define MAXLIBSEQLEN 100000

/* Max number of library pairs */
#define MAXPAIR 100

/* Normalization factor mult*/
#define NORMF 1000
/* #define NORMF 10000 */

/* Number of fields for a pair aligned*/
#define PAIRFIELDS 7
...
/* Relaxation cycles for the library */
#define NCYCLES 1

/* Library score factor, put it in parameter file? */
#define LIBFACTOR 1
...
/* Alternative structure tcoffee-like*/
typedef struct s_entryLib
{
	long ResA;
	long ResB;
	long SeqB;
...
typedef struct s_packLib
{
	entryLib** entry; //#match
	long nPairs;
	long avgScore;

} packLib;

typedef struct s_scoredEntry
{
	long Pos;
	long Score;
} scoredEntry;

typedef struct s_frameEntry
{
	entryLib** Entry;
	long ne;
} frameEntry;
...
void readargv (int argc,char *argv[],
	       ...
	       char *ExonsFile, char* HSPFile, char* LibraryFile, char* GenePrefix);
...
long ReadLib (char * FileName, packExternalInformation* external);
...
entryLib* RequestNewEntryLib();
packLib** RequestMemoryLibrary();
scoredEntry* RequestNewScoredEntry();
...
/* Altering the workflow of geneid for the library mode*/
void copyData();
void UpdateBestGenePredictions(account** multipleAccounts, packEvidence** multipleEvidences,
								packGenes** multipleGenes, account* m, packEvidence* evidence,
								packGenes* genes);

packEvidence** RequestMMemoryEvidences();
packGenes** RequestMMemoryGenes();
account** RequestMMemoryAccounts();

double computeNSD(packGenes** allSeqsGenes);
double computeSd(packGenes** allSeqsGenes);
double computeAvg(packGenes** allSeqsGenes);
int* predictionsLength(packGenes** allSeqsGenes, int totalSeqs);
